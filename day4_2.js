let passports="hcl:#6b5442 ecl:brn iyr:2019 \
...
passports.map( p => {
  let valid_fields = p.fields.filter( f => {
    /* check field is present 
    return validations
    .filter( v => v.field == f.name) 
    .length == 1
    */
    /* check field is valid by regexp */
    let validation = validations.filter( v => v.field == f.name)[0]
    if(!validation) return false;
    const regexp = new RegExp(validation.regexp, "g");
    const matches = f.value.match(regexp);
    const field_is_valid = matches && matches.length > 0
    f.validation = validation;
    f.matches = matches
    f.is_valid = field_is_valid
    return field_is_valid;
  })
  const passport_is_valid = valid_fields.length >= validations.length;
  p.is_valid = passport_is_valid;
  return p;
})
.filter( p => p.is_valid )
